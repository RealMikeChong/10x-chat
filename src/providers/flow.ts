...
async function openModelSelector(page: Page): Promise<void> {
    ...
    const fallback = page.locator('button:has-text("Nano Banana"), button:has-text("Video"), button:has-text("Veo")').first();
...
export async function configureVideoMode(
    ...
    const orientBtn = orientation === 'portrait'
